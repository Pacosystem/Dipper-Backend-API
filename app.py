...
app = Flask(__name__)
CORS(app, origins="*")
BUCKET_NAME = "modelos-alojados" # ¡Verificado!
...
def upload_model():
    """
    Endpoint para subir un modelo (.onnx) y CONSTRUIR sus metadatos
    desde los campos del formulario.
    """
    # 1. Validar que tengamos los campos del formulario
    ...
    if 'input_features_json' not in request.form: # <-- ¡NUEVO!
         return jsonify({'error': 'Falta "input_features_json"'}), 400
            
    model_file = request.files['model_file']
    model_name = request.form['model_name'].lower()
    
    try:
        # 2. Construir el diccionario de metadatos desde los campos
        metadata = {
            "description": request.form['description'],
            # El frontend nos envía la lista de features como un string JSON
            "input_features": json.loads(request.form['input_features_json']) 
        }
    except Exception as e:
        return jsonify({'error': f'Error procesando metadatos JSON: {e}'}), 400
    ...
    try:
        ...
        # 4. Subir el .meta (que acabamos de crear)
        blob_meta = bucket.blob(f"{model_name}.meta")
        blob_meta.upload_from_string(
            json.dumps(metadata, indent=2), # Lo guardamos bonito
        ...
        return jsonify({'success': f"Modelo ONNX '{model_name}' y sus metadatos creados."}), 201
...
def root():
    return jsonify({"message": "API de Modelos v3.1 (Formulario Dinámico) está activa"}), 200